      ...
      <div className={`transition-opacity duration-700 ${splashDone ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
          ...
          <div className="relative max-w-7xl mx-auto px-6 pt-32 pb-16 w-full">
            <div className="max-w-4xl">
              ...
              <div className="flex flex-wrap items-center gap-4">
                <Link href="/join" className="group flex items-center gap-3 bg-[#E8191A] hover:bg-[#B81011] px-8 py-4 font-bold tracking-widest uppercase text-sm transition-all hover:shadow-[0_0_40px_rgba(232,25,26,0.4)] clip-corner text-[#F2F2F2]" style={{ fontFamily: 'Barlow Condensed, sans-serif' }}>
                  Try Out Now <ChevronRight size={16} className="group-hover:translate-x-1 transition-transform" />
                </Link>
                <Link href="/teams" className="flex items-center gap-3 border border-white/10 hover:border-white/30 px-8 py-4 text-[#F2F2F2]/70 hover:text-[#F2F2F2] font-medium tracking-wider uppercase text-sm transition-all" style={{ fontFamily: 'Barlow Condensed, sans-serif' }}>
                  Our Rosters
                </Link>
        ...
        <section className="relative py-20 overflow-hidden">
          ...
          <div className="relative max-w-7xl mx-auto px-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {teams.map((team) => (
                <Link key={team.id} href={`/teams#${team.id}`} className="group relative bg-[#141414] border border-white/5 hover:border-white/10 p-6 card-hover overflow-hidden">
                  ...
                  <div className="mb-4">
                    <span className="text-xs font-mono px-2 py-0.5 mb-1 inline-block" style={{ color: team.color, background: `${team.color}15`, border: `1px solid ${team.color}30` }}>{team.region}</span>
              ...
              {news.map((article) => (
                <a key={article.id} href={article.link} target="_blank" rel="noopener noreferrer"
                  ...
                  <div className="h-48 relative overflow-hidden bg-[#1A1A1A]">
                    <div className="absolute inset-0">
                      <Image src={article.image} alt={article.title} fill className="object-cover transition-all duration-500 scale-105 group-hover:scale-100 brightness-[0.25] group-hover:brightness-75" />
                    </div>
        ...
        <div className="relative bg-[#E8191A] py-16 overflow-hidden">
          ...
          <div className="relative max-w-7xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-8">
            ...
            <Link href="/join" className="group flex-shrink-0 flex items-center gap-3 bg-[#F2F2F2] text-[#E8191A] hover:bg-[#0D0D0D] hover:text-[#F2F2F2] px-10 py-5 font-black tracking-widest uppercase text-base transition-all clip-corner" style={{ fontFamily: 'Barlow Condensed, sans-serif' }}>
