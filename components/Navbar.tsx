...
import OvertakeLogo from './OvertakeLogo'
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-3 group">
            <div className="relative">
              {/* Real PNG logo — white on transparent */}
              <div className="relative w-10 h-10 group-hover:drop-shadow-[0_0_8px_rgba(232,25,26,0.7)] transition-all duration-300">
                <Image
            ...
            <span
              className="font-display text-2xl font-black tracking-widest uppercase text-[#F2F2F2]"
            ...
            {navLinks.map((link) => (
              <Link
                ...
                className={`nav-link px-4 py-2 text-sm font-medium tracking-wider uppercase transition-colors ${
                  pathname === link.href
                    ? 'text-[#E8191A]'
                    : 'text-[#F2F2F2]/55 hover:text-[#F2F2F2]'
                }`}
      ...
      >
        ...
        <div
          className={`absolute top-0 right-0 h-full w-72 bg-[#141414] border-l border-white/5 flex flex-col transition-transform duration-300 ${
            menuOpen ? 'translate-x-0' : 'translate-x-full'
          }`}
        >
          <div className="flex items-center justify-between p-6 border-b border-white/5">
            <div className="flex items-center gap-3">
              <OvertakeLogo size={28} />
              <span
                className="font-display text-lg font-black tracking-widest text-[#E8191A]"
                style={{ fontFamily: 'Barlow Condensed, sans-serif' }}
              >
                OVERTAKE
              </span>
